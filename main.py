...
import t_bot_db as tbotdb


config = tbotdb.load_config('config.json')
DSN = tbotdb.dsn(config)
engine = tbotdb.sq.create_engine(DSN)
tbotdb.create_db(engine)
Session = tbotdb.sessionmaker(bind=engine)
session = Session()

tbotdb.import_words(session)
...
known_users = tbotdb.get_users(session)
userStep = {}
buttons = []
...
def create_cards(message, step=0):
    ...
    if cid not in known_users:
        tbotdb.add_new_user(session, cid)
        known_users.append(cid)
        userStep[cid] = 0
        bot.send_message(cid, f"Привет, новый пользователь {message.from_user.username}, Начнём изучать английский язык!")
    else:
        if step == 0: bot.send_message(cid, f"Привет, {message.from_user.username}, я тебя уже знаю!")
    markup = types.ReplyKeyboardMarkup(row_width=2)

    global buttons
    buttons = []
    word_tuple = tbotdb.get_random_word(session, cid)
    if not word_tuple:
    ...
    others = tbotdb.get_other_words(session, cid, target_word)
...
def delete_word(message):
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        tbotdb.delete_word(session, message.from_user.id, data['translate_word'])
...
def get_ru_word(message):
    ...
    if message.text == Command.CANCEL:
        ...
        bot.register_next_step_handler(message, get_ru_word)
    elif tbotdb.is_cyrillic(message.text):
        target_word = tbotdb.translate_word(message.text, token=tbotdb.ya_token)
        markup = types.ReplyKeyboardMarkup(row_width=2)
...
def get_en_word(message, word, target_word):
    ...
    if message.text == Command.CANCEL:
        ...
        elif tbotdb.is_english(message.text):
...
def add_word_to_db(tid, word, translate):
    ...
    resp = tbotdb.add_new_word(session, tid, word, translate)
    if resp == 1:
